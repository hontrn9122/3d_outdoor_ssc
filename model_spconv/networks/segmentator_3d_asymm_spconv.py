...
class PointSegBlock(nn.Module):
    def __init__(
        self,
        in_feature,
        num_classes,
    ):
        super(PointSegBlock, self).__init__()
...
class Asymm_3d_spconv(nn.Module):
    def __init__(self,
        ...
        self.point_head = PointSegBlock(4 * init_size + 256, nclasses)


    def encode(self, voxel, point_feat, point, batch_size):
        ret = self.downCntx(voxel)
        down1c, down1b = self.resBlock2(ret)
        down2c, down2b = self.resBlock3(down1c)
        down3c, down3b = self.resBlock4(down2c)
        down4c, down4b = self.resBlock5(down3c)
        
        return down2b, down3b, down4b
    
        
    def forward(self, voxel, point_feat, p2v):
        # x = x.contiguous()
        # coors = coors.int()
        # import pdb
        # pdb.set_trace()
        ...
        point_feat = torch.cat([point_feat, up0e.features[p2v]], dim=1)
        ppred = self.point_head(point_feat)
        # print(ppred.shape)
        
        return y, ppred, [down2b, down3b, down4b]
